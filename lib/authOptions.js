...
// Whitelisted testing-phase friends — comma-separated in the env var.
// Auto-assigned the 'tester' tier on sign-in (see signIn callback below).
// Only applied when the user is currently on 'free' — never overwrites a
// tier that was set manually for some other reason (e.g. pro/annual).
    ...
    async signIn({ user, account }) {
      ...
      try {
        ...
        if (existingUser) {
          ...
          if (existingUser.tier === "free" && isTesterEmail(existingUser.email)) {
            existingUser.tier = "tester";
            changed = true;
          }
          ...
          await User.create({
            ...
            tier: isTesterEmail(user.email) ? "tester" : "free",
          });
